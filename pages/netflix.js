import React from 'react'
import Layout from '@/layout/Layout'
import Header from '@/components/Header'
import Link from 'next/link'

const netflix = () => {
  return (
    <>
      <Layout pagina={'Netflix'}>
        <Header/>
                
        <div className="h-screen flex flex-col items-center justify-center"  style={{ backgroundColor: '#141414' }}>
          <h1 className="text-gray-200 text-5xl text-center">¿Quién está viendo ahora?</h1>
          <div className="flex flex-row flex-wrap gap-5 mt-8">
            <Link href="/netflix-play" className="flex flex-col items-center group gap-2">
              <img
                className="rounded border-2 border-transparent group-hover:border-2 group-hover:border-gray-300 py-2"
                src="https://occ-0-2210-2433.1.nflxso.net/dnm/api/v6/K6hjPJd6cR6FpVELC5Pd6ovHRSk/AAAABY5cwIbM7shRfcXmfQg98cqMqiZZ8sReZnj4y_keCAHeXmG_SoqLD8SXYistPtesdqIjcsGE-tHO8RR92n7NyxZpqcFS80YfbRFz.png?r=229"
                width="143"
                height="143"
                alt="Imagen"
              />
              <p className="text-gray-500 group-hover:text-gray-300"> Carlos Jerez </p>
            </Link>

            <Link href="/netflix-agregar-perfil" className="flex flex-col items-center group gap-3 py-1">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-[150px] w-[150px] group-hover:bg-gray-300 border-2 border-transparent" viewBox="0 0 20 20" fill="#6b7280">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z" clip-rule="evenodd" />
              </svg>
              <p className="text-gray-500 group-hover:text-gray-300"> Agregar Perfil </p>
            </Link>

          </div>


          <button className="border-2 border-gray-600 text-gray-600 px-4 py-1 mt-20 hover:border-gray-400 hover:text-gray-400">
            Administrar Perfiles
          </button>

        </div>
      </Layout>
    </>
  )
  }

  export default netflix